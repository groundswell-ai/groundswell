...
describe('@Step decorator with retry options', () => {
  ...
  it('should include error field in stepRetry event', async () => {
    // Arrange
    const events: WorkflowEvent[] = [];

    class RetryWithErrorWorkflow extends Workflow {
      attemptCount = 0;
    ...
    // Act
    const wf = new RetryWithErrorWorkflow();
    await wf.run();
    ...
    if (retryEvents[0]?.type === 'stepRetry') {
      ...
      expect(retryEvents[0].error.message).toBe('Test error for retry');
      expect(retryEvents[0].error.workflowId).toBe(wf.id);
    }
  });
});
